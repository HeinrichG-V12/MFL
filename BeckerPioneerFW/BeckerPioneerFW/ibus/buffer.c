...
uint8_t tx_buffer[TX_MESSAGE_COUNT][MESSAGE_LENGHT];
uint8_t tx_producer_ptr, tx_consumer_ptr;
...
{
	tx_buffer[tx_producer_ptr][pos] = data;
}
...
void tx_buffer_insertEntry (void)
{
	...
	if (tx_producer_ptr == TX_MESSAGE_COUNT)
	{
		tx_producer_ptr = 0;
	}
}

uint8_t* tx_buffer_get_entry (void)
{
	if(tx_producer_ptr == tx_consumer_ptr)
	return NULL;
	else
	return tx_buffer[tx_consumer_ptr];
}
...
void tx_buffer_remove_entry (void)
{
	...
	if (tx_consumer_ptr == TX_MESSAGE_COUNT)
	{
		tx_consumer_ptr = 0;
	}
}